...
def get_agent_name(model_type, gamma, beam_size, guess_predictor, on_topic, reward):
    agent_name = model_type + '_gamma' + re.sub('\.', '_', str(gamma)) + '_reward' + re.sub('\.', '_', str(reward))
...
def main():
    create_policies_filename = '../scripts/create_policies_reward_v2.sh'
    condor_submit_init_filename = '../scripts/condor_submit_init_reward_v2.sh'
    condor_submit_train_filename = '../scripts/condor_submit_train_reward_v2.sh'
    condor_submit_test_filename = '../scripts/condor_submit_test_reward_v2.sh'
    ...
    for (model_type, gamma, beam_size, guess_predictor, on_topic, reward) in settings:
        ...
        create_condor_script(agent_name + '_init', 'condorized_init_policy_' + str(reward) + '.sh', agent_name + ' static')
        condor_submit_init_file.write('condor_submit ' + condor_scripts_dir + agent_name + '_init.sh\n')

        # Condor script to train
        create_condor_script(agent_name + '_train', 'condorized_train_policy_' + str(reward) + '.sh', agent_name)
