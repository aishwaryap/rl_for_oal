...
def get_agent_name(model_type, gamma, beam_size, guess_predictor, on_topic, reward):
    agent_name = model_type + '_gamma' + re.sub('\.', '_', str(gamma))
...
def main():
    create_policies_filename = '../scripts/create_policies_fixedlen_v2.sh'
    condor_submit_init_filename = '../scripts/condor_submit_init_fixedlen_v2.sh'
    condor_submit_train_filename = '../scripts/condor_submit_train_fixedlen_v2.sh'
    condor_submit_test_filename = '../scripts/condor_submit_test_fixedlen_v2.sh'

    model_types = ['linear', 'mlp']
    gamma = [0.9, 1.0]
    beam_size = [10, 100, None]
    guess_predictor = [True, False]
    on_topic = [False]
    rewards = [200, 250, 300, 350, 400, 450, 500]

    settings = list(itertools.product(model_types, gamma, beam_size, guess_predictor, on_topic, rewards))
