...
def get_agent_name(model_type, gamma, beam_size, guess_predictor, on_topic, alpha):
    agent_name = model_type + '_gamma' + re.sub('\.', '_', str(gamma)) + '_alpha' + re.sub('\.', '_', str(alpha))
...
def main():
    create_policies_filename = '../scripts/create_policies_reinforce.sh'
    condor_submit_init_filename = '../scripts/condor_submit_init_reinforce.sh'
    condor_submit_train_filename = '../scripts/condor_submit_train_reinforce.sh'
    condor_submit_test_filename = '../scripts/condor_submit_test_reinforce.sh'

    model_types = ['reinforce']
    gamma = [1.0]
